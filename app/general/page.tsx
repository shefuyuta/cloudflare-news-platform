...
export default async function GeneralPage({ searchParams }: {
  searchParams: Promise<{ region?: string; q?: string; tag?: string | string[]; hours?: string }>;
}) {
  ...
  const [items, available] = await Promise.all([
    listArticles(env, { category: "general", region: sp.region, q: sp.q, tags, hoursAgo, limit: 60 }),
