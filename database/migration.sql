...
-- ---- 8. Digests (Daily / Weekly / Monthly AI summaries) --------------
-- Generated by a single daily cron (09:00 JST) that always writes a
-- 'daily' row, and additionally writes 'weekly' on Fridays and 'monthly'
-- on the last day of the month. One LLM call per digest produces both
-- languages (content is stored as JSON: {"ja":"...","en":"..."}).
CREATE TABLE IF NOT EXISTS digests (
  id           TEXT PRIMARY KEY,   -- `${type}-${period_end}` e.g. "daily-2026-08-14"
  type         TEXT NOT NULL CHECK (type IN ('daily','weekly','monthly')),
  period_start TEXT NOT NULL,      -- ISO date
  period_end   TEXT NOT NULL,      -- ISO date (the day the digest was generated for)
  content      TEXT NOT NULL,      -- JSON: {"ja": "...", "en": "..."}
  stats        TEXT,               -- JSON: the raw aggregate numbers used, for transparency/debugging
  generated_at TEXT DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS idx_digests_type ON digests(type, period_end DESC);
